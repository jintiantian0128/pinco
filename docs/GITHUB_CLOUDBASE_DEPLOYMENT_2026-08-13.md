...
4. CLI 确认“版本创建成功”后，即使后续日志轮询异常，工作流仍会等待公网结果；先要求 `/health` 返回 HTTP 200、`version=0.7.0`、`release_sha` 等于本次 GitHub SHA，且 DeepSeek/MySQL 同时在线，再连续三轮执行一次性账号闭环与三轮真实 Agent 职业记忆/重进历史恢复。账号测试会自动注销和清理数据。
